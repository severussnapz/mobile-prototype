...
8. **ApplicationStack RGD must be published (blocking).** The schema group is
   confirmed by `emisx-platform-engineering`: instances are served under
   `kro.run/v1alpha1` (migrated off the old `genesis.io` group) — the manifest
   uses this. The remaining dependency is delivery: the reconciled RGD that
   serves `kro.run/v1alpha1` **and** carries both `enableRDS` and `enableBedrock`
   must be merged to platform `main` and published to the cluster before the
   stack passes admission. Track with `emisx-platform-engineering`.
