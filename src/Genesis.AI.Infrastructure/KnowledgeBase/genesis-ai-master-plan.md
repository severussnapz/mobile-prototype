...
- [x] **Root cause established July 2026 (PROVEN from code, no session logs required) — it is a two-stage trap, not a single threshold.** The registry is `is_published = true` and passes `GetProjectArtefactManifestAsync`; `requirements/` is a P06 foundation prefix; `AppendFoundationArtefactsAsync` has no size cap. So the registry DOES reach `FoundationService`. But `BuildCachedSectionContent` applies its own 50,000-char threshold and injects an outline whose header reads: *"OUTLINE (file too large for full caching — use `get_artefact` for full content)"*. The agent follows that instruction exactly. `BuildGetArtefactResult` then applies a second 50,000-char threshold and returns its own outline, whose header reads: *"Use `search_in_artefact` to retrieve specific sections, or `edit_artefact` directly using selectors from this outline."* `search_in_artefact` is DOM search (`PrototypeDomSearchMatch`, fragment paths, CSS selectors) and does not work on markdown; there are no selectors in a `.md` file. **Foundation routes the agent to `get_artefact`; `get_artefact` routes it to a tool that cannot read the file. Dead end.** The agent never ignored an instruction — it followed both precisely.
- [x] Compounding defect: `BuildFileOutline` is **not an outline for markdown**. For any non-HTML/CSS extension it returns `content[..2000] + "[...truncated...]"` — a blind head-truncation presented to the agent under a "STRUCTURAL OUTLINE" header. The CSO agent received 2,000 chars of a 122KB clinical hazard registry, twice (once from foundation, once from the tool), each time labelled as structure.
- [x] Compounding defect: `BuildFileOutline` is **duplicated** — one copy in `FoundationService`, one in `ConversationStreamController` — both carrying the same magic `50_000` and the same markdown behaviour. Same bug in two places; a fix to one does not fix the other.
- [x] Scale (measured, dev DB July 2026): four artefacts exceed the 50KB threshold — `requirements/HAZARD-REGISTRY.md` 122.2KB, `requirements/REQ-001-unified-inbound-inbox.md` 82.4KB, `requirements/REQ-007-search-filter-sort-folders.md` 67.8KB, `requirements/REQ-010-workflow-tasks.md` 59.0KB. All four are markdown. All four are foundation artefacts for downstream stages. **The registry is not a special case — three REQ files are being outlined into foundation the same way.**
- [x] This is the silent-seam failure class (§9c): two components, each internally correct and independently tested, and the failure lives in the join — the recovery path one promises is one the other cannot deliver.
...
- [ ] **Full-content bypass is ruled out by measurement.** 122KB is ~30K tokens per read, growing every P06 session. Two or three reads in one conversation is ~90K tokens on a single file. The interim bypass previously proposed here is not viable for this class and is withdrawn.
- [ ] Fix direction: proper resolution is the structure-aware chunker + small-to-big retrieval from the Knowledge Layer plan (match small, inject the parent section) so an agent pulls a specific hazard card by heading path rather than a whole-file outline. **What the agent actually needs is two things, not one:** (1) every HAZ-ID visible, so it cannot allocate a duplicate — a real markdown heading tree, not a head-truncation; (2) full text of only the hazards it is working on — a fetch-section-by-heading path. The outline alone is today's bug with better formatting; the fetch alone is useless because the agent cannot know what to ask for.
- [ ] Prerequisite for any fix: **there is no markdown retrieval tool.** `search_in_artefact` is DOM-only. Until one exists, no outline is safe to return for markdown, because there is no recovery path from it. Needs a real large-file retrieval test — not the InMemory-tested path.
- [ ] Standing rule (from this finding): if a component truncates content and tells the agent to recover via another component, a test must prove that recovery path works for that content type. This is seam-test class 3 (write → read-back) applied to the truncation/recovery seam.
...
**Work order for remainder of Plan 4d (revised July 2026 after contract layer investigation):**

1. **Contract layer — BLOCKED, do not start.** `ContractPinRole` carries `Req` and `Arch` provenance pins, but neither is expressible today: ARCH is a section inside each `REQ-XXX.md`, not its own artefact, and a single Req pin cannot name the ~20 `requirements/REQ-*.md` files a project holds (proven: `NormalisationGateService` regex `^requirements/(REQ-\d{3})[^/]*\.md$`). §9b-i's REQ+ARCH provenance was written for the post-`artefact-scope-restructure` world — a thin REQ index and a real ARCH artefact — which is Plan 5 work. `ContractManifest.Create` derives its required set from `Enum.GetValues<ContractPinRole>()`, so the pins cannot be left unpopulated. Options: (a) drop Req/Arch to four contract pins now, restore provenance in Plan 5; (b) relax the one-pin-per-role invariant to allow many Req pins. **Undecided — needs a design sitting, not an implementation session.** The four contract-file pins are correct and unaffected either way.
2. **Help chat query context fix** — `HelpChatStreamService` passes raw `message` to both `IKnowledgeService.QueryAsync` calls with no conversation history, so vague follow-ups ("why are we doing it") embed with no subject anchor and match whatever scores highest globally. `conversation.Messages` is already loaded and in scope. Fix: build the retrieval query from the prior **user** message verbatim + current message (`{prior_user_message}: {current_message}`), not an LLM summary — no extra round-trip, no new failure mode. Small, no design questions, bug proven in live use.
3. **Engineering hygiene** — DTO mapping completeness backfill, systemic error handling audit, NSwag type generation, controller completeness check, JSON field name integration tests, enum serialisation tests, behavioural tests (same-turn edit guard, post-search read block, zero-match block), ToolExecutionContext extraction, migration chain CI check against empty Postgres.
4. **Pipeline Prompt Quality Review** — full P01–P11 audit.

**Closed this session:** `PrototypeSingleFileEnabled` prod flip ✅ · anchor failure root cause ✅ · `get_artefact` truncation root cause established (fix is Workstream C Phase 3, not a patch) ✅
