...
internal static class PrototypeDomSearchDocumentHelper
{
    ...
    internal static async Task<IReadOnlyList<(string FragmentPath, string Content)>> LoadPrototypeFragmentsAsync(
        IArtefactRepository artefactRepository,
        IArtefactStorageService artefactStorageService,
        Guid projectId,
        CancellationToken cancellationToken)
    {
        var allArtefacts = await artefactRepository.GetByProjectIdAsync(projectId, cancellationToken);
        ...
        {
            var content = await artefactStorageService.GetContentAsync(artefact.S3Key, cancellationToken);
