...
public class ConversationsController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return CreatedAtAction(nameof(GetConversation), new { id = conversationId }, new { data = resource });
        }
        catch (InvalidOperationException ex)
        {
            return Conflict(new
            {
                errors = new[]
                {
                    new { status = "409", title = "Stage prerequisite not met", detail = ex.Message }
                }
            });
        }
    }
    ...
    public async Task<IActionResult> GetConversation(Guid id, CancellationToken cancellationToken)
    {
        ...
        if (conversation is null)
        {
            return NotFound(new
            {
                errors = new[]
                {
                    new { status = "404", title = "Conversation not found", detail = $"No conversation found with ID '{id}'." }
                }
            });
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Created($"/api/v1/conversations/{id}/messages/{messageId}", new
        {
